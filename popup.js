...
function getRemediation(issue) {
  ...
  const remediationMap = {
    ...
    "document-title": "Add a descriptive and meaningful title to the webpage.",

    "duplicate-id": "Ensure that every id attribute on the page is unique.",

    "aria-allowed-attr":
      "Remove unsupported ARIA attributes or use ARIA attributes that are valid for the element's role.",

    "aria-roles": "Use a valid ARIA role that is appropriate for the element.",
...
function appendSummaryCards(parent, response) {
  ...
  const cards = [
    ...
    [
      ...
      "🧩",
      "Affected Elements",
      getCount(response.affectedElements),
    ],
  ];
...
function createIssueCard(issue, index) {
  ...
  if (wcagCriteria || wcagLevel) {
    ...
    if (wcagCriteria) {
      wcagInfo.appendChild(
        createElement("span", "wcag-criteria", `WCAG ${wcagCriteria}`),
      );
    }
    if (wcagLevel) {
      wcagInfo.appendChild(
        createElement("span", "wcag-level", `Level ${wcagLevel}`),
      );
    }
  ...
  issueCard.appendChild(
    createElement("div", "issue-title", `${index + 1}. ${issueTitle}`),
  );
  ...
  // Issue Description
  issueCard.appendChild(
    createElement("div", "issue-description", issueDescription),
  );
  const remediationBox = createElement("div", "remediation-box");

  remediationBox.appendChild(
    createElement("div", "remediation-title", "💡 How to Fix"),
  );

  remediationBox.appendChild(
    createElement("div", "remediation-text", remediation),
  );

  issueCard.appendChild(remediationBox);

  // Highlight Element + Previous / Next
  if (Array.isArray(issue?.nodes) && issue.nodes.length > 0) {
    const highlightWrapper = createElement("div", "issue-highlight-wrapper");

    const highlightButton = createElement(
      "button",
    ...
    const navigationWrapper = createElement("div", "highlight-navigation");
    ...
    function updateHighlight() {
      const currentNode = issue.nodes[currentNodeIndex];
      const targets = currentNode?.target;

      highlightElement(targets);

      counter.textContent = `${currentNodeIndex + 1} / ${issue.nodes.length}`;

      previousButton.disabled = currentNodeIndex === 0;
      nextButton.disabled = currentNodeIndex === issue.nodes.length - 1;
    }
    ...
    nextButton.addEventListener("click", () => {
      if (currentNodeIndex < issue.nodes.length - 1) {
        currentNodeIndex++;
        updateHighlight();
      }
    });

    navigationWrapper.append(previousButton, counter, nextButton);

    highlightWrapper.append(highlightButton, navigationWrapper);
...
function renderResults(response) {
  ...
  resultContainer.appendChild(
    createElement("h3", null, "Accessibility Report"),
  );
...
exportBtn.addEventListener("click", () => {
  ...
  const report = {
    ...
    summary: {
      ...
      affectedElements: scanResults.affectedElements,
    },
